...
import { errorString, escapeText, extend } from './utils.mjs';
...
export function start(QUnit) {
  ...
  function abortTests() {
    const abortButton = /** @type {HTMLButtonElement} */ (id('qunit-abort-tests-button'));
    if (abortButton) {
  ...
  function interceptNavigation(ev) {
    ...
    const filterInputElem = /** @type {HTMLInputElement} */ (id('qunit-filter-input'));
  ...
  function getUrlConfigHtml() {
    ...
    for (let i = 0; i < urlConfig.length; i++) {
      ...
      let val = /** @type {UrlConfig} */ (config.urlConfig[i]);
      ...
      if (tests) {
        ...
        if ('checked' in field && field.checked) {
          for (let i = 0; i < length; i++) {
            const test = /** @type {Element} */ (children[i]);
  ...
  function applyUrlParams() {
    const filter = /** @type {HTMLInputElement} */ (id('qunit-filter-input')).value;
  ...
  /**
   * @param {Fuzzysort.KeyResults<ModuleDetails>} results from fuzzysort
   * @return {string} HTML
   */
  ...
  function toolbarModuleFilter(beginDetails) {
    ...
    dropdownData = {
      ...
      isDirty: function () {
        return (
          [...dropdownData.selectedMap.keys()].sort().join(',') !==
    ...
    function selectionChange(evt) {
      ...
      const textForm = dropdownData.selectedMap.size
        ? dropdownData.selectedMap.size +
          ' ' +
          (dropdownData.selectedMap.size === 1 ? 'module' : 'modules')
        : 'All modules';
