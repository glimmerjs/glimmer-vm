...
abstract class TemplateRangeTests extends RangeTests {
  ...
  renderRange(value: number) {
    this.render.template(`<input ${this.attrs} />`, {
      max: this.max,
      min: this.min,
      value,
    });
...
jitSuite(
  class EmberComponentRangeTests extends RangeTests {
    ...
    renderRange(value: number): void {
      this.register.component('Curly', 'range-input', '', this.component());
      this.render.template(`{{range-input max=this.max min=this.min value=this.value}}`, {
        max: this.max,
        min: this.min,
        value,
      });
...
jitSuite(
  class BasicComponentImplicitAttributesRangeTest extends RangeTests {
    ...
    renderRange(value: number): void {
      this.register.component('Glimmer', 'RangeInput', '<input ...attributes/>');
      this.render.template(`<RangeInput ${this.attrs.replace('%x', value.toString())} />`);
    }
...
jitSuite(
  class BasicComponentSplattributesLastRangeTest extends RangeTests {
    ...
    renderRange(value: number): void {
      this.register.component('Glimmer', 'RangeInput', '<input type="text" ...attributes/>');
      this.render.template(`<RangeInput ${this.attrs.replace('%x', value.toString())} />`);
    }
