...
export class AttributesTests extends BrowserRenderTest {
  ...
  static {
    let desc = Reflect.getOwnPropertyDescriptor(
      AttributesTests.prototype,
      'disable updates properly'
    );
    console.log({ desc, value: desc?.value, isTest: desc?.value?.isTest });
  }
  ...
  //   this.assert.strictEqual(this.readDOMAttr('value'), '', `readDOMAttr('value')`);
  //   this.assertStableRerender();
  ...
  //   this.assert.strictEqual(this.readDOMAttr('value'), 'bar', 'manually updated value');

  //   model.value = 'foo';
  //   this.rerender();
  //   this.assert.strictEqual(this.readDOMAttr('value'), 'foo', 'updated tracked value');
  //   this.assertStableNodes();
