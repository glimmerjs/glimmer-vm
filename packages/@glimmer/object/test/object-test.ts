...
function isDirty<T>(ref: PathReference<T>, newValue: T) {
  QUnit.assert.ok(ref.value() === newValue, (<any>ref).label() + " has new value " + newValue);
}

function isClean<T>(ref: PathReference<T>): void;
function isClean() {
  // clean references are allowed to report dirty
}

function allDirty<T>(refs: PathReference<T>[], newValue: T) {
  refs.forEach(function(ref) { isDirty(ref, newValue); });
}

function allClean<T>(refs: PathReference<T>[]) {
