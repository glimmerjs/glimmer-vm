...
const { ImportsNotUsedAsValues, JsxEmit, ModuleKind, ModuleResolutionKind, ScriptTarget } =
  typescriptLibrary;
...
export class Package {
  ...
  get #external() {
     ...
     */
    return (id) => {
      if (id === 'tslib' || id.startsWith('@babel/runtime')) {
        return INLINE;
      }
      ...
      if (id.startsWith('@glimmer/') || id.startsWith('@simple-dom/')) {
        return EXTERNAL;
      }

      switch (id) {
        case '@handlebars/parser':
        case 'simple-html-tokenizer':
        case 'babel-plugin-debug-macros':
          return EXTERNAL;
      }
