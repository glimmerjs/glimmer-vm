...
import { MachineOp, Op, Optional } from '@glimmer/interfaces';
...
export function opcodeMetadata(op: MachineOp | Op, isMachine: 0 | 1): Optional<NormalizedMetadata> {
  let value = isMachine ? MACHINE_METADATA[op] : METADATA[op];

  return value || null;
}

const METADATA: Optional<NormalizedMetadata>[] = fillNulls(Op.Size);
const MACHINE_METADATA: Optional<NormalizedMetadata>[] = fillNulls(MachineOp.Size);
