...
import { EntityParser, TokenizerState } from 'simple-html-tokenizer';

import print from '../generation/print';
import { voidMap } from '../generation/printer';
import { gelog } from '../geUtils';
...
export class TokenizerEventHandlers extends HandlebarsNodeVisitors {
  ...
  createMustacheTag(mustache: ASTv1.MustacheStatement): void {
    this.currentNode = {
      type: 'StartTag',
      name: '',
      attributes: [],
  ...
  closeMustacheTag(mustache: ASTv1.MustacheStatement): void {
    this.currentNode = {
      type: 'EndTag',
      name: '',
      attributes: [],
  ...
  assembleConcatenatedValue(
    parts: (ASTv1.MustacheStatement | ASTv1.BlockStatement | ASTv1.TextNode)[]
  ...
  assembleAttributeValue(
    ...
    isDynamic: boolean,
    span: SourceSpan
  ): ASTv1.ConcatStatement | ASTv1.MustacheStatement | ASTv1.BlockStatement | ASTv1.TextNode {
    if (isDynamic) {
      if (isQuoted) {
