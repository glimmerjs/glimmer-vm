...
import { Dict, Opaque, Option, unsafe, SymbolTable, BlockSymbolTable } from '@glimmer/interfaces';
...
import { CheckPathReference, CheckCompilableBlock } from '../compiled/opcodes/-debug-strip';
...
export class Arguments implements IArguments {
  ...
  setup(stack: EvaluationStack, names: string[], blockNames: string[], positionalCount: number, synthetic: boolean) {
    ...
    */
    ...
    let blocksBase = positionalBase - (blocksCount * 2);
  ...
  get length(): number {
    return this.positional.length + this.named.length + (this.blocks.length * 2);
  }
...
export class BlockArguments implements IBlockArguments {
  ...
  get values(): (SymbolTable | VMHandle)[] {
    ...
    if (!values) {
      let { base, length, stack } = this;
      values = this.internalValues = stack.slice<VMHandle>(base, base + length * 2);
    }
  ...
  get(name: string): Option<ScopeBlock> {
    ...
    let table = check(stack.get(idx * 2, base), CheckOption(CheckBlockSymbolTable));
    let handle = check(stack.get(idx * 2 + 1, base), CheckOption(CheckOr(CheckHandle, CheckCompilableBlock)));

    return handle === null ? null : [handle, table!];
  }
...
class CapturedBlockArguments implements ICapturedBlockArguments {
  ...
  constructor(
    public names: string[],
    public values: (SymbolTable | VMHandle)[]
  ) {
    this.length = names.length;
  }
  ...
  get(name: string): Option<ScopeBlock> {
    ...
    return [this.values[idx * 2 + 1] as VMHandle, this.values[idx * 2] as BlockSymbolTable];
  }
}
