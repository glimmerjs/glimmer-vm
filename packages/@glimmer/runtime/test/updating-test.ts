...
module("[glimmer-runtime] Updating", hooks => {
  ...
  test("triple curlies with empty string initial value", () => {
    ...
    equalTokens(root, '<div><!----></div>', "back to empty");
  });

  test("triple curlies in a table", () => {
    let input = {
      value: ''
    };
    let template = compile(stripTight`
    ...
    input.value = '<tr><td>Bold and spicy</td></tr>';
    rerender();
